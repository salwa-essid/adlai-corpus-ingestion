...
| Script | Purpose |
|---|---|
| `node scripts/loadtest.js [--clones-per-source N]` | Spec section 9 load test: clones the real corpus `N`x (default 10) into synthetic sources, runs them through the real ingestion pipeline with embeddings stubbed, reports throughput, then removes every synthetic row/file it created. (Renamed 2026-08-19 from `load-test.js` — that name matches Node's `node --test` auto-discovery glob `*-test.js`, so `npm test` was silently running this DB-mutating load test as if it were a unit test and reporting it "failed" whenever no DB was configured.) |
| `node scripts/expand-eval-questions.js` | Auto-generates up to 2 extra `eval_questions` per real source (picked from evenly-spaced articles, skipping ones already cited) so a domain isn't scored off a single lucky/unlucky question. These are content-based smoke tests, not attorney-authored — flagged as needing attorney review before being trusted for the cutover gate (spec section 7). |
...
- **Eval question set**: the runner works end-to-end, but needs a real attorney-authored question set per domain before its numbers mean anything for a cutover decision. Today most domains have only 1-3 auto-generated smoke-test questions (`scripts/expand-eval-questions.js` — content-based, not attorney-authored). A CSV import tool for attorney-authored questions (attorney writes in a spreadsheet, imports without touching the DB directly) does not exist yet — this README previously claimed it did; it needs to be built before real attorney-authored questions can be added at scale.
- **AI Watch**: diff detection and the notification loop work; `llm_impact_analysis` now calls Cohere chat for a real diff-specific summary (falls back to the old rule-based stub if `COHERE_API_KEY` is unset or the call fails — see `src/services/aiWatchService.js`). Notification delivery is still a console log, not a real channel (email/webhook/Slack) — that part remains a gap.
